...
def test_docker_image(tp):
    ...
    stdout_path = _write_artifact(artifact_dir, f"{base}.stdout.txt", result.stdout or "")
    stderr_path = _write_artifact(artifact_dir, f"{base}.stderr.txt", result.stderr or "")
    ...
    prefill_match = re.search(r"Prefill\. latency:\s*([\d\.eE+-]+)\s*s,\s*throughput:\s*([\d\.eE+-]+)\s*token/s", output)
    assert prefill_match is not None, "No latency/throughput numbers found after 'Prefill'"
    ...
    assert decode_match is not None, "No latency/throughput numbers found after 'Decode'"